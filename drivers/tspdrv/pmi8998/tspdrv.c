...
asmlinkage void _Pmi8998DbgOut(int level, const char *fmt,...)
{
...
asmlinkage static void _Pmi8998DbgOutV(int level, const char *fmt,va_list args)
{
    static char printk_buf[MAX_DEBUG_BUFFER_LENGTH];
    static char prefix[6][4] =
        {" * ", " ! ", " ? ", " I ", " V", " O "};
...
asmlinkage void _Pmi8998DbgOutTemp(const char *fmt,...)
{
    int nDbgLevel = atomic_read(&g_nDebugLevel);
    if (DBL_TEMP <= nDbgLevel)
    {
        va_list args;
        va_start(args, fmt);
        _Pmi8998DbgOutV(DBL_TEMP, fmt, args);
        va_end(args);
    }
}

asmlinkage void _Pmi8998DbgOutFatal(const char *fmt,...)
{
    int nDbgLevel = atomic_read(&g_nDebugLevel);
    if (DBL_FATAL <= nDbgLevel)
    {
        va_list args;
        va_start(args, fmt);
        _Pmi8998DbgOutV(DBL_FATAL, fmt, args);
        va_end(args);
    }
}

asmlinkage void _Pmi8998DbgOutErr(const char *fmt,...)
{
    int nDbgLevel = atomic_read(&g_nDebugLevel);
    if (DBL_ERROR <= nDbgLevel)
    {
        va_list args;
        va_start(args, fmt);
        _Pmi8998DbgOutV(DBL_ERROR, fmt, args);
        va_end(args);
    }
}

asmlinkage void _Pmi8998DbgOutWarn(const char *fmt,...)
{
    int nDbgLevel = atomic_read(&g_nDebugLevel);
    if (DBL_WARNING <= nDbgLevel)
    {
        va_list args;
        va_start(args, fmt);
        _Pmi8998DbgOutV(DBL_WARNING, fmt, args);
        va_end(args);
    }
}

asmlinkage void _Pmi8998DbgOutInfo(const char *fmt,...)
{
    int nDbgLevel = atomic_read(&g_nDebugLevel);
    if (DBL_INFO <= nDbgLevel)
    {
        va_list args;
        va_start(args, fmt);
        _Pmi8998DbgOutV(DBL_INFO, fmt, args);
        va_end(args);
    }
}

asmlinkage void _Pmi8998DbgOutVerbose(const char *fmt,...)
{
    int nDbgLevel = atomic_read(&g_nDebugLevel);
    if (DBL_VERBOSE <= nDbgLevel)
    {
        va_list args;
        va_start(args, fmt);
        _Pmi8998DbgOutV(DBL_VERBOSE, fmt, args);
        va_end(args);
    }
}

asmlinkage void _Pmi8998DbgOutOverkill(const char *fmt,...)
{
    int nDbgLevel = atomic_read(&g_nDebugLevel);
    if (DBL_OVERKILL <= nDbgLevel)
    {
        va_list args;
        va_start(args, fmt);
        _Pmi8998DbgOutV(DBL_OVERKILL, fmt, args);
        va_end(args);
    }
}
...
static ssize_t write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
    ...
    */
    ...
    if (!Pmi8998SaveOutputData(g_cWriteBuffer, count))
    {
        DbgOutErr(("tspdrv: Pmi8998SaveOutputData failed.\n"));
        return -EIO;
    }
