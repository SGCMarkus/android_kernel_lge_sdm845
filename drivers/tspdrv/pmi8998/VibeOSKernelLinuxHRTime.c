...
#endif
struct semaphore Pmi8998_g_hSemaphore;
...
static enum hrtimer_restart VibeOSKernelTimerProc(struct hrtimer *timer)
{
    ...
    if (VibeSemIsLocked(&Pmi8998_g_hSemaphore))
    {
        up(&Pmi8998_g_hSemaphore);
    }
...
static void VibeOSKernelLinuxStartTimer(void)
{
    ...
    if (!g_bTimerStarted)
    {
        ...
        sema_init(&Pmi8998_g_hSemaphore, NUM_EXTRA_BUFFERS);
...
#endif
    {
        ...
        */
        res = down_interruptible(&Pmi8998_g_hSemaphore);  /* wait for the semaphore to be freed by the timer */
        if (res != 0)
        {
    ...
    ** interrupt will release the Pmi8998_g_hSemaphore while VibeOSKernelProcessData is executing and the player
    ...
    */
    ...
    if (g_bTimerStarted && !VibeSemIsLocked(&Pmi8998_g_hSemaphore))
    {
        int res;

        res = down_interruptible(&Pmi8998_g_hSemaphore);
...
static void VibeOSKernelLinuxTerminateTimer(void)
{
    ...
    if (VibeSemIsLocked(&Pmi8998_g_hSemaphore)) up(&Pmi8998_g_hSemaphore);
}
