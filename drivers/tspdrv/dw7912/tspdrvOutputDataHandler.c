...
**     Dw7912SendOutputData() is called.
**
...
int Dw7912SaveOutputData(const char *outputDataBuffer, int count)
{
    int i = 0;
...
#error "SPI_HEADER_SIZE expected to be 3"
    ...
    {
        DbgOutErr(("Dw7912SaveOutputData: invalid write buffer size.\n"));
        return 0;
    }
    ...
    {
        DbgOutErr(("Dw7912SaveOutputData: outputDataBuffer invalid.\n"));
        return 0;
    }
    ...
    while (i < count)
    {
        ...
        if ((i + SPI_HEADER_SIZE) > count)
        {
            ...
            ** (Should never happen).
            */
            DbgOutFatal(("Dw7912SaveOutputData: invalid buffer index.\n"));
        }

        /* Check bit depth */
        if (8 != pInputBuffer->nBitDepth)
        {
            DbgOutWarn(("Dw7912SaveOutputData: invalid bit depth. Use default value (8).\n"));
        }
        ...
        if ((i + SPI_HEADER_SIZE + pInputBuffer->nBufferSize) > count)
        {
            ...
            ** (Should never happen).
            */
            DbgOutFatal(("Dw7912SaveOutputData: invalid data size.\n"));
        }
        
        /* Check actuator index */
        if (NUM_ACTUATORS <= pInputBuffer->nActuatorIndex)
        {
            DbgOutErr(("Dw7912SaveOutputData: invalid actuator index.\n"));
...
bool Dw7912SendOutputData(void)
{
    int i;
    bool ret = 0;
    ...
    for (i = 0; i < NUM_ACTUATORS; i++) 
    {
...
#endif
        {
            ...
            /* DbgOutErr(("Dw7912SendOutputData: 2: Size, BitDepth: %d, Buffer: %d, dataBuffer[0]:%d\n", 
...
void Dw7912ResetOutputData(void)
{
    int i;
...
bool Dw7912IsOutputDataBufferEmpty(void)
{
    int i;
