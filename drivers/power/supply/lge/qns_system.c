...
static int qns_get_fcc_design(int *fcc, int *design)
{
	...
	if (fcc != NULL) {  /* fcc = full charging count */
		if(power_supply_get_property(data.battery_psy,
...
static struct class_attribute qnovo_attributes[] = {
	[CHARGING_STATE] = __ATTR_RO(charging_state),
	[CURRENT_NOW]    = __ATTR_RO(current_now),
	[VOLTAGE]        = __ATTR_RO(voltage),
	[TEMP]           = __ATTR_RO(temp),
	[FCC]            = __ATTR_RO(fcc),
	[DESIGN]         = __ATTR_RO(design),
	[SOC]            = __ATTR_RO(soc),
	[BATTERY_TYPE]   = __ATTR_RO(battery_type),
	[CHARGE_CURRENT] = __ATTR(charge_current, 0660, NULL, charge_current_store),
	[CHARGE_VOLTAGE] = __ATTR(charge_voltage, 0660, NULL, charge_voltage_store),
	[OPTIONS]        = __ATTR_RW(options),
	[ALARM]          = __ATTR_RW(alarm),
	__ATTR_NULL,
};
