...
extern bool fusb251_complete_probe(void);
#endif
...
#ifdef CONFIG_LGE_USB_MOISTURE_FUSB251
	bool complete_usb = fusb251_complete_probe();
#else
	bool complete_usb = true;
#endif
	if (!instance || !complete_usb)
		return false;
...
static struct gpio_desc *workaround_ds_en_gpio = NULL;
static struct gpio_desc *workaround_load_sw_on_gpio = NULL;
void workaround_control_vbus2_regulator(struct smb_charger *chg, bool on)
{
	if (!workaround_ds_en_gpio || !workaround_load_sw_on_gpio)
		return;

	if (on) {
		gpiod_set_value(workaround_ds_en_gpio, 1);
		gpiod_set_value(workaround_load_sw_on_gpio, 1);
	} else {
		gpiod_set_value(workaround_load_sw_on_gpio, 0);
		gpiod_set_value(workaround_ds_en_gpio, 0);
	}
}
...
void workaround_control_vbus2_regulator_trigger(void)
{
	...
	if (smblib_vbus_regulator_is_enabled(chg->vbus_vreg->rdev)) {
		pr_info("[W/A] CVR) Disable Vbus2 by BCL.\n");
		gpiod_set_value(workaround_load_sw_on_gpio, 0);
		gpiod_set_value(workaround_ds_en_gpio, 0);
		gpiod_direction_output(workaround_ds_en_gpio, 0);
	}
}
...
void workaround_control_vbus2_regulator_init(struct smb_charger *chg)
{
	...
	if (!gpio_is_valid(gpio)) {
		pr_err("[W/A] CVR) Fail to get gpio\n");
		return;
	}
	workaround_ds_en_gpio = gpio_to_desc(gpio);
	gpiod_direction_output(workaround_ds_en_gpio,
			gpiod_get_value(workaround_ds_en_gpio));
	...
	workaround_load_sw_on_gpio = gpio_to_desc(gpio);
	gpiod_direction_output(workaround_load_sw_on_gpio,
			gpiod_get_value(workaround_load_sw_on_gpio));
}


////////////////////////////////////////////////////////////////////////////
// LGE Workaround : Skip USB ID with DS
////////////////////////////////////////////////////////////////////////////

extern bool unified_bootmode_usermode(void);
static bool workaround_enable_skip_usb_id = false;
static bool workaround_hall_ic = false;

void workaround_update_hall_ic(bool hall_ic)
{
	workaround_hall_ic = hall_ic;
}

bool workaround_skip_usb_id_running(struct smb_charger *chg)
{
	if (!workaround_enable_skip_usb_id)
		return false;

	if (workaround_hall_ic && unified_bootmode_usermode())
		return true;
	return false;
}
...
{
	struct device_node *node = chg->dev->of_node;
	workaround_enable_skip_usb_id = of_property_read_bool(node, "lge,enable-skip-usb-id");
}
