...
extern bool msm_act_data_enqueue(uint32_t reg_addr, uint32_t reg_data, struct cam_actuator_ctrl_t *a_ctrl);
		...
		struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	//CAM_INFO(CAM_ACTUATOR, "[AFHALL] ACT_HALL : %u soc_id : %d", act_hall, a_ctrl->soc_info.index);
