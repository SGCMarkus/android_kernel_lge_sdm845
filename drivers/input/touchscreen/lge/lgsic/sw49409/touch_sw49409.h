...
#ifndef LGE_TOUCH_SW49409_H
#define LGE_TOUCH_SW49409_H
...
#define tc_ic_status			(0x200)	/* sw49409_touch_info base addr*/
...
#define Q_TOUCH_SENSE			(0xC64)
#define QCOVER_ACT_AREA_START		(0xC65)
#define QCOVER_ACT_AREA_END		(0xC66)

#define SPR_CHARGER_CTRL		(0xC68)
#define SPR_GLOVE_CTRL			(0xC69)
#define SPR_GRAP_CTRL			(0xC6A)
#define SPR_ROTATION_CTRL		(0xC6C)
//#define SPR_IME_CTRL			(0xC61)
...
/* Firmware control */
#define sys_rst_ctl			(0x006)//ok
#define sys_boot_ctl			(0x00E)//ok
#define sys_sram_ctl			(0x00F)//ok

#define fw_boot_code_addr		(0x03F)	//(0x0BD)

#define spr_code_offset			(0x0AA)//ok
#define spr_data_offset			(0x0AF)//ok
...
#define code_access_addr		(0xFD0)//ok
...
#define FW_BOOT_LOADER_INIT		0x74696E69	//"init" //?? check fw
#define FW_BOOT_LOADER_CODE		0x544F4F42	//"BOOT" //?? check fw

#define FLASH_KEY_CODE_CMD		0xDFC1		//?? check fw
#define FLASH_KEY_CONF_CMD		0xE87B		//?? check fw
#define FLASH_KEY_OTPM_CMD      	0xAE71		//?? check fw
#define FLASH_BOOTCHK_VALUE		0x0A0A0000	//?? check fw

#define CFG_MAGIC_CODE			0xCACACACA
#define CFG_CHIP_ID			49409
...
#define IC_STATUS_NORMAL_MASK		0x00000010	/* (1<<4) */
#define IC_STATUS_GLOBAL_RESET_BIT 	0x00003E00	/* (1<<9)|(1<<10)|(1<<11)|(1<<12)|(1<<13) */
#define IC_STATUS_HW_RESET_BIT		0x000000A6	/* (1<<01)|(1<<02)|(1<<05)|(1<<07) */
...
#define TC_STATUS_NORMAL_MASK		0x085080E0	/* (1<<05)|(1<<06)|(1<<07)|
							   (1<<15)|(1<<20)|(1<<22)|(1<<27) */
#define TC_STATUS_GLOBAL_RESET_BIT	0x80000000	/* (1<<31) */
#define TC_STATUS_HW_RESET_BIT		0x000006C0	/* (1<<06)|(1<<07)|(1<<09)|(1<<10) */
#define TC_STATUS_SW_RESET_BIT		0x00000000	/* NULL */
#define TC_STATUS_FW_UPGRADE_BIT	0x00000000	/* NULL */
#define TC_STATUS_LOGGING_BIT		0x0000A000	/* (1<<13)|(1<<15) */
...
struct sw49409_touch_debug_info {
...
struct sw49409_touch_info {
	u32 ic_status;
	u32 tc_status;
	u32 wakeup_type:8;
	u32 touch_cnt:5;
	u32 button_cnt:3;
	u32 palm_bit:16;	//12byte
	struct sw49409_touch_data data[10];	//120byte
} __packed;

/* test control */
struct sw49409_fw_info {
	u8 version[2];
	u8 product_id[8];
	u8 image_version[2];
...
struct sw49409_asc_info {
	u16 normal_s;
	u16 acute_s;
	u16 obtuse_s;
};

struct sw49409_active_area {
	s16 x1;
	s16 y1;
	s16 x2;
	s16 y2;
...
struct sw49409_swipe_ctrl {
	...
	struct sw49409_active_area start;
	u8 wrong_dir_thres;
	u8 init_rat_chk_dist;
	u8 init_rat_thres;
	struct sw49409_active_area border;
	bool debug_enable;
};

struct sw49409_lpwg_abs_ctrl {
	bool enable;
	struct sw49409_active_area area;
	struct sw49409_active_area border;
	s16 offset_y;
...
struct sw49409_data {
	struct device *dev;
	struct kobject kobj;
	struct sw49409_touch_info info;
	struct sw49409_touch_debug_info debug_info;
	struct sw49409_fw_info fw;
	struct sw49409_swipe_ctrl swipe[4];	/* left, right, up, down */
	struct sw49409_lpwg_abs_ctrl lpwg_abs;
...
static inline struct sw49409_data *to_sw49409_data(struct device *dev)
{
	return (struct sw49409_data *)touch_get_device(to_touch_core(dev));
}

static inline struct sw49409_data *to_sw49409_data_from_kobj(struct kobject *kobj)
{
	return (struct sw49409_data *)container_of(kobj,
			struct sw49409_data, kobj);
}
static inline int sw49409_read_value(struct device *dev,
		u16 addr, u32 *value)
{
	return sw49409_reg_read(dev, addr, value, sizeof(*value));
}

static inline int sw49409_write_value(struct device *dev,
		u16 addr, u32 value)
{
	return sw49409_reg_write(dev, addr, &value, sizeof(value));
}

#endif /* LGE_TOUCH_SW49409_H */
