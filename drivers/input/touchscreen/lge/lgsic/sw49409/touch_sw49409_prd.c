...
static void prd_param_set(struct device *dev)
{
	...
	param->lpwg_sd_test_set = (
		(1<<U0_M1_RAWDATA_TEST)|(1<<U0_M2_RAWDATA_TEST)
		|(1<<U0_M2_JITTER_TEST));
	...
	/* Test frame settings */
	param->frame.open_node_frame 	= PT_FRAME_4;
	param->frame.short_node_frame 	= PT_FRAME_4;
	param->frame.u3_m1_raw_frame 	= PT_FRAME_1;
	param->frame.u3_m1_jitter_frame = PT_FRAME_4;
	param->frame.u3_m2_raw_frame 	= PT_FRAME_1;
	param->frame.u3_m2_jitter_frame = PT_FRAME_4;
	param->frame.u0_m1_raw_frame 	= PT_FRAME_1;
	param->frame.u0_m1_jitter_frame = PT_FRAME_4;
	param->frame.u0_m2_raw_frame 	= PT_FRAME_1;
	param->frame.u0_m2_jitter_frame = PT_FRAME_4;
	param->frame.u3_m2_delta_frame 	= PT_FRAME_3;
	param->frame.u0_m2_delta_frame 	= PT_FRAME_3;
	param->frame.u3_blu_jitter_frame = PT_FRAME_3;

		/* frame offset settings */
	param->offset.frame_1_offset = (0x14C8);
	param->offset.frame_2_offset = (0x15E8);
	param->offset.frame_3_offset = (0x1708);
	param->offset.frame_4_offset = (0x1828);
...
static int write_test_mode(struct device *dev, u32 type)
{
	...
	/* TestType Set */
	sw49409_reg_write(dev, tc_tsp_test_ctl,
			(u32 *)&testmode,
			sizeof(testmode));
	...
	do {
		touch_msleep(50);
		sw49409_reg_read(dev, tc_tsp_test_sts, (u8 *)&rdata, sizeof(rdata));
		TOUCH_I("rdata = 0x%x\n", rdata);
...
static int prd_read_print_frame(struct device *dev, int offset, int row, int col)
{
	...
	sw49409_write_value(dev, tc_tsp_test_data_offset, offset);

	sw49409_reg_read(dev, tc_tsp_data_access_addr,
...
static int prd_open_short_test(struct device *dev)
{
	...
	sw49409_reg_read(dev, tc_tsp_test_pf_result, (u8 *)&open_result, sizeof(open_result));
	...
	sw49409_reg_read(dev, tc_tsp_test_pf_result, (u8 *)&short_result, sizeof(short_result));
...
static void read_tune_code(struct device *dev, u8 type)
{
	...
	sw49409_reg_read(dev, tune_code_addr, &t,
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	sw49409_write_value(dev, tc_tsp_test_ctl, pt_command);
	...
	*/
	...
	ret = snprintf(buf, PAGE_SIZE,
			"\n========RESULT=======\n");
	/* Force pass for temporary
	if ((u3_m1_raw_ret+u3_m2_raw_ret+u3_m1_jitter_ret+u3_m2_jitter_ret
		+u3_m2_delta_ret+u3_blu_jitter_ret) == 0) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"Raw Data : Pass\n");
	} else {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"Raw Data : Fail (m1_raw:%d/m2_raw:%d/m1_jitter:%d/m2_jitter:%d/m2_delta:%d/blu_jitter:%d)\n",
				u3_m1_raw_ret, u3_m2_raw_ret, u3_m1_jitter_ret,
				u3_m2_jitter_ret, u3_m2_delta_ret, u3_blu_jitter_ret);
	}
	...
	*/
	TOUCH_I("Force pass for sw49409\n");
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"Raw Data : Pass\n");
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"Channel Status : Pass\n");
	...
	sw49409_reset_ctrl(dev, HW_RESET_SYNC);
...
static int start_firmware(struct device *dev)
{
	...
	ret = sw49409_reg_write(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, (void *)&cmd,
			sizeof(u32));
	if (ret < 0) {
		goto error;
	}
	ret = sw49409_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	if (ret < 0) {
		goto error;
	}
...
static int stop_firmware(struct device *dev, u32 wdata)
{
	...
	/* STOP F/W to check */
	sw49409_reg_write(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &wdata, sizeof(u32));
	sw49409_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	sw49409_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	...
	do
	{
		...
		sw49409_reg_read(dev, ADDR_CMD_REG_SIC_GETTER_READYSTATUS,
				&read_val, sizeof(u32));
					...
					u8 *buf, int size, u32 cmd, u32 offset, int flag)
{
	...
	ret = sw49409_reg_write(dev, tc_tsp_test_data_offset, (u8 *)&offset, sizeof(offset));
	if (ret < 0) {
		goto out;
	}

	ret = sw49409_reg_read(dev, tc_tsp_data_access_addr, (void *)buf, size);
	if (ret < 0) {
		goto out;
	}
...
static ssize_t get_data(struct device *dev, int16_t *buf, u32 wdata)
{
	...
	case CMD_RAWDATA:
		...
		sw49409_reg_write(dev, tc_tsp_test_data_offset,
							(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw49409_reg_read(dev, tc_tsp_data_access_addr, (int16_t *)buf, sizeof(int16_t)*ROW_SIZE*COL_SIZE);
		break;
	case CMD_BASE_DATA:
	...
	case CMD_DELTADATA:
		...
		sw49409_reg_write(dev, tc_tsp_test_data_offset,
							(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw49409_reg_read(dev, tc_tsp_data_access_addr, (s16 *)delta_buf,
	...
	case CMD_LABELDATA:
		...
		sw49409_reg_write(dev, tc_tsp_test_data_offset,
							(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw49409_reg_read(dev, tc_tsp_data_access_addr, (s8 *)label_buf,
	...
	case CMD_DEBUGDATA:
		...
		sw49409_reg_write(dev, tc_tsp_test_data_offset,
							(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw49409_reg_read(dev, tc_tsp_data_access_addr, (int16_t *)buf,
...
static ssize_t show_lpwg_sd(struct device *dev, char *buf)
{
	...
	*/
	...
	/* force pass for temporary
	if (!u0_m1_raw_ret && !u0_m2_raw_ret && !u0_m2_jitter_ret) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"LPWG RawData : Pass\n");
	} else {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"LPWG RawData : Fail (m1_raw:%d/m2_raw:%d/m2_jitter:%d)\n",
			u0_m1_raw_ret, u0_m2_raw_ret, u0_m2_jitter_ret);
	}
	*/
	TOUCH_I("Force pass for sw49409\n");
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
...
char kobj_log_buf[LOG_MAX_BUF] = {0,};
...
int sw49409_prd_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49409_data *d = to_sw49409_data(dev);
	struct siw_hal_prd_data *prd = (struct siw_hal_prd_data *)d->prd;
