...
//Use BR ENHANCE mode instead of daylight(SRE) mode.
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	sscanf(buf, "%d", &input);
	panel->lge.br_enhance = input;

	if (panel->lge.ddic_ops && panel->lge.ddic_ops->lge_br_enhance_set)
		panel->lge.ddic_ops->lge_br_enhance_set(panel, panel->lge.br_enhance);
...
int lge_color_manager_create_sysfs(struct dsi_panel *panel, struct device *panel_sysfs_dev)
{
	...
	if ((rc = device_create_file(panel_sysfs_dev, &dev_attr_ecc_status)) < 0)
		pr_err("add ecc set node fail!");
