...
static int lge_br_enhance_set_sw43103(struct dsi_panel *panel, int input)
{
	mutex_lock(&panel->panel_lock);

	pr_info("Brightness enhance mode (%d)\n", input);

	if(panel->lge.br_enhance)
		lge_ddic_dsi_panel_tx_cmd_set(panel, LGE_DDIC_DSI_BR_ENHANCE_ON);
	else
		lge_ddic_dsi_panel_tx_cmd_set(panel, LGE_DDIC_DSI_BR_ENHANCE_OFF);
...
static void lge_vr_lp_mode_set_sw43103(struct dsi_panel *panel, int input)
{
	...
	panel->lge.vr_low_persist_enable = input;
	enable = !!panel->lge.vr_low_persist_enable;

	if (enable) {
		rc = lge_ddic_dsi_panel_tx_cmd_set(panel, LGE_DDIC_DSI_SET_VR_MODE_PRE_ON);
	} else {
...
int lge_vr_lp_mode_enable_sw43103(struct dsi_panel *panel)
{
	int rc = 0;
	lge_vr_lp_mode_set_sw43103(panel, 1);
	return rc;
}

int  lge_vr_lp_mode_disable_sw43103(struct dsi_panel *panel)
{
	int rc = 0;
	lge_vr_lp_mode_set_sw43103(panel, 0);
	return rc;
}
...
struct lge_ddic_ops sw43103_ops = {
	...
	.lge_br_enhance_set = lge_br_enhance_set_sw43103,
	/* image quality */
	...
	.vr_enable = lge_vr_lp_mode_enable_sw43103,
	.vr_disable = lge_vr_lp_mode_disable_sw43103,
